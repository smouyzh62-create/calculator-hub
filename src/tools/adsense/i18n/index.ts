...
export function detectLocale(): Locale { return 'en'; }

function getT(locale: Locale): (key: string) => string {
  return (key: string) => {
...
export function t(key: string, locale?: Locale): string {
  const l = locale || (typeof window !== 'undefined' ? (localStorage.getItem('hub-locale') as Locale | null) : null) || 'en';
  return getT(l === 'zh' ? 'zh' : 'en')(key);
}
