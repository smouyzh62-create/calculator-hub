    ...
    <header className="flex items-center justify-between mb-8">
      ...
      <div className="flex items-center gap-3">
        <motion.button initial={{opacity:0,scale:0.8}} animate={{opacity:1,scale:1}} transition={{duration:0.4,delay:0.15}} onClick={toggleLocale}
          className="relative w-12 h-7 rounded-full transition-colors flex items-center px-0.5" style={{backgroundColor: locale==='en' ? '#6366f1' : '#22c55e'}}>
          <motion.div animate={{x: locale==='en' ? 0 : 20}} transition={{type:'spring',stiffness:300,damping:20}}
            className="w-6 h-6 rounded-full bg-white shadow flex items-center justify-center text-[10px] font-bold" style={{color: locale==='en' ? '#6366f1' : '#22c55e'}}>{locale==='en'?'EN':'中'}</motion.div>
        </motion.button>
